...
/**
 * MasterAdminRoleChecker Component
 * 
 * This component allows users to check if a specific address has master admin privileges
 * by calling the hasMasterAdminRole function.
 */
interface MasterAdminRoleCheckerProps {
  adminContract: any; // Contract for checking master admin role
  onCheckComplete?: (result: boolean, address: string) => void; // Optional callback
}

const MasterAdminRoleChecker = ({
  adminContract,
  onCheckComplete
}: MasterAdminRoleCheckerProps) => {
  ...
  useEffect(() => {
    console.log('MasterAdminRoleChecker state:', {
      accountToCheck,
      lastChecked,
      isFormValid,
      errorMessage,
  ...
  const {
    ...
    ...adminContract,
    functionName: 'hasMasterAdminRole',
  ...
  // Check master admin role for the address
  const handleCheckRole = () => {
    ...
    refetchRoleCheck().then((result) => {
      ...
      setErrorMessage('Error checking master admin role: ' + error.message);
    });
  };
  ...
  return (
    ...
    >
      ...
      <div className="bg-gray-700/30 rounded-lg p-4 mb-4">
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Enter the Ethereum address to check for master admin role privileges
          </p>
        </div>
        ...
        {isFormValid && hasRoleData !== undefined && !isRoleCheckError && (
          <div className={`w-full rounded-lg p-4 flex items-center justify-between ${
            ...
            <div className="flex items-center">
              {hasRoleData ? (
                <>
                  <div className="w-3 h-3 bg-purple-500 rounded-full mr-3 relative">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-purple-400 opacity-75"></span>
                  </div>
                  <div>
                    <h4 className="text-md font-medium text-purple-400">Master Admin Role Granted</h4>
                    <p className="text-xs text-gray-300 mt-0.5">
                      {accountToCheck.substring(0, 6)}...{accountToCheck.substring(accountToCheck.length - 4)} has master admin privileges
                    </p>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div>
                    <h4 className="text-md font-medium text-red-400">No Master Admin Role</h4>
                    <p className="text-xs text-gray-300 mt-0.5">
                      {accountToCheck.substring(0, 6)}...{accountToCheck.substring(accountToCheck.length - 4)} does not have master admin privileges
                    </p>
                  </div>
                </>
              )}
              ...
              <div>
                ...
                <p className="text-xs mt-1">
                  {roleCheckError 
                    ...
                    : 'An error occurred while checking the master admin role. Please try again.'}
                </p>
              </div>
            </div>
          </div>
                ...
                {checkHistory.map((item, index) => (
                  <li key={index} className="py-2 first:pt-0 last:pb-0">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        <div className={`w-2 h-2 rounded-full ${item.hasRole ? 'bg-purple-500' : 'bg-red-500'} mr-2`}></div>
                        <button
                      ...
                      <div className="flex items-center">
                        <span className={`text-xs ${item.hasRole ? 'text-purple-400' : 'text-red-400'} mr-3`}>
                          {item.hasRole ? 'Master Admin' : 'Not Master Admin'}
                        </span>
      ...
      <div className="mt-6 pt-4 border-t border-gray-700">
        <h4 className="text-sm font-medium text-gray-300 mb-2">About Master Admin Role</h4>
        <p className="text-sm text-gray-400 mb-3">
          The Master Admin Role Checker allows you to verify if a specific Ethereum address has been granted master admin privileges in the system.
        </p>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
          <div className="bg-gray-700/20 rounded-md p-3">
            <h5 className="text-xs font-medium text-amber-400 mb-1">Master Admin Privileges</h5>
            <p className="text-xs text-gray-400">
              Addresses with master admin privileges have the highest level of access in the system, including the ability to grant or revoke other admin roles, manage critical system parameters, and execute emergency functions.
            </p>
          </div>
          
          <div className="bg-gray-700/20 rounded-md p-3">
            <h5 className="text-xs font-medium text-amber-400 mb-1">Security Implications</h5>
            <p className="text-xs text-gray-400">
              The master admin role represents a significant privilege level and should be granted with extreme caution. These accounts should be secured with multisig wallets or hardware-based security solutions.
            </p>
          </div>
        </div>
        
        <div className="bg-gray-700/20 rounded-md p-3 mb-3">
          <h5 className="text-xs font-medium text-amber-400 mb-1">Role Hierarchy</h5>
          <p className="text-xs text-gray-400 mb-2">
            The platform typically implements a role hierarchy:
          </p>
          <ul className="text-xs text-gray-400 space-y-2">
            <li className="flex items-start">
              <span className="text-purple-400 mr-2">•</span>
              <span><strong>Master Admin:</strong> Highest level with complete system control</span>
            </li>
            <li className="flex items-start">
              <span className="text-green-400 mr-2">•</span>
              <span><strong>Admin:</strong> Platform-wide administrative capabilities with limited critical functions</span>
            </li>
            <li className="flex items-start">
              <span className="text-blue-400 mr-2">•</span>
              <span><strong>School Admin:</strong> Administrative capabilities limited to specific school operations</span>
            </li>
            <li className="flex items-start">
              <span className="text-gray-400 mr-2">•</span>
              <span><strong>User:</strong> Standard user with no administrative capabilities</span>
            </li>
          </ul>
        </div>
        
        <p className="text-xs text-gray-400">
          This role check verifies permissions against the blockchain's current state. Master Admin privileges are critical to system governance and should be monitored regularly for security audits.
        </p>
      </div>
    </motion.div>
  );
...
export default MasterAdminRoleChecker;