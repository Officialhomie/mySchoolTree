...
// Define the structure for the organization initialization parameters
type OrganizationInitParams = {
...
// Default values for form fields (empty strings)
const DEFAULT_PARAMS: OrganizationInitParams = {
  revenueSystem: '',
  studentProfile: '',
  tuitionSystem: '',
  masterAdmin: '',
...
const OrganizationInitialization = ({ contract }: { contract: any }) => {
  // Connected wallet account
  const { address: connectedAddress } = useAccount();
  
  // Input states
  const [params, setParams] = useState<OrganizationInitParams>(DEFAULT_PARAMS);
  ...
  // Use connected address as default for admin addresses
  useEffect(() => {
    if (connectedAddress) {
      if (!params.masterAdmin) {
        setParams(prev => ({ ...prev, masterAdmin: connectedAddress }));
      }
      if (!params.organizationAdmin) {
        setParams(prev => ({ ...prev, organizationAdmin: connectedAddress }));
      }
    }
  }, [connectedAddress, params.masterAdmin, params.organizationAdmin]);
  ...
  const handleAddressChange = (value: string, field: keyof OrganizationInitParams) => {
  ...
  useEffect(() => {
    const requiredAddresses = Object.values(params);
  ...
  useEffect(() => {
    if (isConfirmed && hash) {
      setStatusMessage(`Organization successfully initialized!`);
  ...
  const handleInitializeRequest = () => {
    ...
    setStatusMessage('You are about to initialize the Organization. Please review the details and confirm.');
  ...
  const handleInitializeConfirm = async () => {
    try {
      ...
      writeContract({
        ...
        args: [
          params.revenueSystem,
          params.studentProfile,
          params.tuitionSystem,
          params.masterAdmin,
          params.organizationAdmin
        ]
      });
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-300 text-sm mb-6">
        Initialize the Organization with required contract addresses and administrators.
      </p>
        ...
        <div className="space-y-6">
          <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700">
            <p className="text-sm text-gray-300">
              This will initialize the Organization with the provided addresses and administrators.
              Once initialized, the organization will be ready to manage educational operations.
            </p>
          </div>
          ...
          <div className="bg-gray-800/20 p-4 rounded-lg border border-gray-700">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="revenueSystem" className="block text-sm font-medium text-gray-300 mb-1">
                  Revenue System Address <span className="text-red-400">*</span>
                </label>
                <input
                  id="revenueSystem"
                  type="text"
                  value={params.revenueSystem}
                  onChange={(e) => handleAddressChange(e.target.value, 'revenueSystem')}
                  placeholder="0x..."
                  disabled={isPending || isConfirming}
                  className={`bg-gray-800 border ${errors.revenueSystem ? 'border-red-500' : 'border-gray-600'} text-gray-200 text-sm rounded-lg focus:ring-blue-500 focus:border-purple-500 block w-full p-2.5 disabled:opacity-60`}
                />
                {errors.revenueSystem && (
                  <p className="mt-1 text-xs text-red-400">{errors.revenueSystem}</p>
                )}
              ...
              <div>
                <label htmlFor="studentProfile" className="block text-sm font-medium text-gray-300 mb-1">
                  Student Profile Address <span className="text-red-400">*</span>
                </label>
                <input
                  id="studentProfile"
                  type="text"
                  value={params.studentProfile}
                  onChange={(e) => handleAddressChange(e.target.value, 'studentProfile')}
                  placeholder="0x..."
                  disabled={isPending || isConfirming}
                  className={`bg-gray-800 border ${errors.studentProfile ? 'border-red-500' : 'border-gray-600'} text-gray-200 text-sm rounded-lg focus:ring-blue-500 focus:border-purple-500 block w-full p-2.5 disabled:opacity-60`}
                />
                {errors.studentProfile && (
                  <p className="mt-1 text-xs text-red-400">{errors.studentProfile}</p>
                )}
                <p className="mt-1 text-xs text-gray-500">
              ...
              <div>
                <label htmlFor="organizationAdmin" className="block text-sm font-medium text-gray-300 mb-1">
                  Organization Admin Address <span className="text-red-400">*</span>
                </label>
                <input
                  id="organizationAdmin"
                  type="text"
                  value={params.organizationAdmin}
                  onChange={(e) => handleAddressChange(e.target.value, 'organizationAdmin')}
                  placeholder="0x..."
                  disabled={isPending || isConfirming}
                  className={`bg-gray-800 border ${errors.organizationAdmin ? 'border-red-500' : 'border-gray-600'} text-gray-200 text-sm rounded-lg focus:ring-blue-500 focus:border-purple-500 block w-full p-2.5 disabled:opacity-60`}
                />
                {errors.organizationAdmin && (
                  <p className="mt-1 text-xs text-red-400">{errors.organizationAdmin}</p>
                )}
                <p className="mt-1 text-xs text-gray-500">
                  The address that will be assigned as organization administrator (defaults to your connected wallet)
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-900/30 border border-blue-700/50 rounded-md p-4">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-800/50 rounded-md p-3">
                <h4 className="text-md font-medium text-blue-300 mb-2">Contract Addresses & Administrators</h4>
                
                <div className="space-y-2">
                  <div>
                    <p className="text-xs text-gray-400">Revenue System:</p>
                    <p className="text-sm font-mono text-gray-300 break-all">{params.revenueSystem}</p>
                  </div>
                  
                  <div>
                    <p className="text-xs text-gray-400">Student Profile:</p>
                    <p className="text-sm font-mono text-gray-300 break-all">{params.studentProfile}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-mono text-gray-300 break-all">{params.organizationAdmin}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-md">
              <p className="text-sm text-yellow-400">
                ...
                <strong>Important:</strong> This operation will initialize the Organization with the specified addresses and administrators. You should only perform this operation once. Make sure all addresses are correct before proceeding.
              </p>
            </div>
          </div>
      ...
      {confirmationState === 'confirmed' && (
        <div className="space-y-6">
          <div className="bg-green-900/30 border border-green-700/50 rounded-md p-4 text-center">
            ...
            <h3 className="text-lg font-semibold text-green-400">Organization Initialized</h3>
            <p className="text-sm text-gray-300 mt-2">
              The Organization has been successfully initialized with your specified addresses and administrators.
              The system is now ready to manage educational operations.
            </p>
          </div>
          ...
          <div className="bg-blue-900/20 border border-blue-700/30 rounded-md p-4">
            ...
            <ul className="space-y-2 text-sm text-gray-300">
              <li className="flex items-start">
                ...
                <span>Set up organization profile and structure</span>
              </li>
              <li className="flex items-start">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-blue-400 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
                <span>Configure tuition and payment systems</span>
              </li>
              <li className="flex items-start">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-blue-400 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
                <span>Create educational programs and offerings</span>
              </li>
              <li className="flex items-start">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-blue-400 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
                <span>Invite teachers and administrators to join</span>
              </li>
            </ul>
          </div>
...
export default OrganizationInitialization;