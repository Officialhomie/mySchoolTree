...
const RevokeRoleComponent = ({ contract }: { contract: any }) => {
  // Form state
  ...
  // Read contract to check role before revoking
  const { 
    data: hasRoleResult,
    ...
    args: [getRoleValue(), accountAddress],
    enabled: processingState === 'verifying' && !!getRoleValue() && !!accountAddress
  });
  ...
  useEffect(() => {
    if (processingState === 'verifying' && !isRoleCheckPending) {
      ...
      if (hasRoleResult === false) {
        setStatusMessage(`Account ${accountAddress} does not have the specified role. No action needed.`);
      ...
      if (hasRoleResult === true) {
        setIsVerified(true);
        setStatusMessage(`Account has the role. Ready to revoke.`);
        setStatusType('info');
        setShowStatus(true);
        setProcessingState('initial');
      }
    }
  }, [hasRoleResult, isRoleCheckPending, isRoleCheckError, roleCheckError, processingState, accountAddress]);

  // Handle transaction success
  useEffect(() => {
    if (isConfirmed && hash) {
      setStatusMessage(`Role successfully revoked!`);
  ...
  useEffect(() => {
    setIsVerified(false);
  }, [selectedRole, customRole, accountAddress, useCustomRole]);
  ...
  const handleVerifyRole = async () => {
    try {
      ...
      setStatusMessage('Checking if account has role...');
      setStatusType('info');
      setShowStatus(true);
  ...
  // Handle the revoke role process
  const handleRevokeRole = async () => {
    try {
      ...
      setStatusMessage('Submitting transaction to revoke role...');
      setStatusType('info');
      setShowStatus(true);
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-300 text-sm mb-6">
        Remove specific role access from a user account.
      </p>
      ...
      {isRoleCheckPending && processingState === 'verifying' && (
        <div className="flex flex-col items-center justify-center p-4 border border-gray-700 rounded-md bg-gray-800/50 mb-6">
          ...
          <p className="text-xs text-gray-400 text-center">
            Querying the blockchain to verify if the account has the specified role.
          </p>
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700">
            <div className="flex items-start">
              ...
              <p className="ml-3 text-sm text-gray-300">
                This operation will permanently revoke the specified role from the user account.
                The user will immediately lose all access and permissions associated with this role.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-3">
              ...
              />
              <label htmlFor="useCustomRole" className="block text-sm font-medium text-gray-300">
          ...
          <div className="bg-red-500/10 border border-red-500/30 rounded-md p-4">
            <div className="flex items-start">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-medium text-red-400">Warning</h3>
                <p className="mt-1 text-sm text-gray-300">
                  This action cannot be undone. Once a role is revoked, the user will immediately lose
                  all privileges associated with that role. Only administrators should perform this action.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              disabled={isRoleCheckPending || !getRoleValue() || !accountAddress || !isValidEthAddress(accountAddress)}
              className={`py-3 px-4 rounded-md text-white font-medium shadow-lg transition-all duration-300 ${
                'bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700'
              } ${
                isRoleCheckPending || !getRoleValue() || !accountAddress || !isValidEthAddress(accountAddress)
          ...
          {isVerified && (
            <div className="p-3 bg-green-500/20 border border-green-500/30 rounded-md text-green-400 text-sm">
              ✓ Verification successful! The account currently has this role and can be revoked.
            </div>
          )}
        </div>
      )}
      ...
      {processingState === 'completed' && (
        <div className="space-y-6">
          <div className="bg-green-900/30 border border-green-700/50 rounded-md p-4 text-center">
            ...
            <h3 className="text-lg font-semibold text-green-400">Role Successfully Revoked</h3>
            <p className="text-sm text-gray-300 mt-2">
              The role has been revoked from the specified account.
